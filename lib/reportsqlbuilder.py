...
class ReportSqlBuilder(object):
    ...
    def _getTotalsSelectClause(self):
        # get multi_value_triggers
        multi_value_triggers = []
        for control in self.params.controls:
            if control.get('multi_value_trigger') and \
               control.value and \
               control.type == 'multi_menu' and \
               len(control.value) > 1:
                multi_value_triggers.append(control.multi_value_trigger)

        select = []
        for c in self.reportColumns.getSelectedColumns():
            if c.get('no_summary_totals'):
                s = "''"
            elif c.get('no_summary_if') in multi_value_triggers:
                s = "''"
